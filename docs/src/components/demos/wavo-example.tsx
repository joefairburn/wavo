...
const WavoExample = () => {
  ...
  const generatedCode = useMemo(() => {
    const lines: string[] = [];
    lines.push('import { Waveform } from "wavo";');
    lines.push("");
    lines.push("const data = [0.2, 0.5, 0.8, 0.6, 0.3, 0.9, 0.4, 0.7];");
    lines.push("");
    lines.push("function MyWaveform() {");
    lines.push("  return (");
    lines.push("    <Waveform dataPoints={data} progress={0.5}>");

    if (controls.renderMode === "bars") {
      const barProps: string[] = [];
      if (controls.gap !== 0) barProps.push(`gap={${controls.gap}}`);
      if (controls.width !== 1) barProps.push(`width={${controls.width}}`);
      if (controls.radius !== 0) barProps.push(`radius={${controls.radius}}`);
      if (controls.optimized) barProps.push("optimized");
      const propsStr = barProps.length > 0 ? ` ${barProps.join(" ")}` : "";
      lines.push(`      <Waveform.Bars${propsStr} />`);
    } else {
      const pathProps: string[] = [];
      if (controls.gap !== 0) pathProps.push(`gap={${controls.gap}}`);
      if (controls.width !== 1) pathProps.push(`width={${controls.width}}`);
      if (controls.smooth) pathProps.push("curvature={0.1}");
      const propsStr = pathProps.length > 0 ? ` ${pathProps.join(" ")}` : "";
      lines.push(`      <Waveform.Path${propsStr} />`);
    }

    if (controls.useGradient) {
      const gradient = createGradient(controls.color);
      const gradientStr = `[${gradient.map((s) => `{ offset: "${s.offset}", color: "${s.color}" }`).join(", ")}]`;
      lines.push(`      <Waveform.Progress gradient={${gradientStr}} />`);
    } else {
      lines.push(`      <Waveform.Progress color="${controls.color}" />`);
    }
  ...
  return (
    <div className="border-2 border-white/10 bg-[#050505]" style={cssVariables}>
      ...
      <div className="flex flex-col">
        {/* Waveform Area */}
        <div className="relative flex h-[200px] w-full items-center justify-center overflow-hidden border-b border-white/10 bg-black">
          {/* Waveform */}
          <Waveform
        ...
        {/* Code Output */}
        <div className="border-t border-white/10 [&_figure]:m-0 [&_figure]:rounded-none [&_figure]:border-0">
          <DynamicCodeBlock lang="tsx" code={generatedCode} />
        </div>
      </div>
    </div>
  );
