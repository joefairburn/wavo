 ...
 * @returns A path string for SVG
 */
const createLinePath = (dataPoints: number[], width: number, gap: number): string => {
  if (!dataPoints.length) return '';
 ...
 * 6. Supports different rendering types (bars or continuous line)
 */
export const Path: React.FC<PathProps> = ({ type = 'bar', width = 3, gap = 1, radius = 2, className }) => {
  ...
  const pathString = useMemo(() => {
    ...
    if (type === 'bar') {
      ...
      return createLinePath(reducedDataPoints, cssBarWidth, cssBarGap);
    }

    return '';
  }, [reducedDataPoints, cssBarWidth, cssBarGap, radius, type]);
